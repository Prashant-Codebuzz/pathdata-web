...
// const EditResourcesIAM = () => {

//     const location = useLocation();
//     const navigate = useNavigate();
//     const tenantId = localStorage.getItem("signin-tenantid");

//     const resourcesIAM = location?.state?.resourcesIAM || {};
...
//         try {
//             const res = await Axios.patch(`/assume_roles`, formData, authorizationHeaders());
...
//         } catch (err) {
//             console.error("Error Patch resourcesIAM++", err);
...
//     useEffect(() => {
//         setFormData({
//             role_name: resourcesIAM?.role_name || "",
//             role_arn: resourcesIAM?.role_arn || ""
//         });
//     }, [resourcesIAM]);
...
//                             <p className="add-title">Edit Resources IAM</p>
//                         </div>
//                     </div>
...
// export default EditResourcesIAM
...
const AddResourcesIAM = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await Axios.patch(`/resource_iam`, data, authorizationHeaders());
    ...
    useEffect(() => {
        if (resourcesIAM) {
            setFormData({
                credential_type: resourcesIAM?.credential_type,
                instance_profile_arn: resourcesIAM?.instance_profile_arn,
                instance_profile_name: resourcesIAM?.instance_profile_name,
                ds_name: resourcesIAM?.ds_name,
                catalog_type: resourcesIAM?.catalog_type,
                credential_role_arn: resourcesIAM?.credential_role_arn,
            });
        }
    }, [resourcesIAM]);
        ...
        <>
            <div className="pd mt-4">
                <div className="add-section row">
                    <div className="col-lg-12">
                        <p className="add-title">Edit Cluster Resource IAM</p>
                    </div>
                </div>
