...
const CredentialConfigure = () => {
    ...
    const createCredentialConfigure = () => {
        navigate("/add-credential-configure")
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await Axios.delete(`/credential_configure`, {
                data: { role_name: deleteId, family_id: familyId },
                headers: {
    ...
    const toggleCheckbox = () => {
        if (isChecked) {
            ...
            setSelectedRows(credentialList.map((_, index) => index));
        }
    ...
    useEffect(() => {
        if (selectedRows.length === credentialList?.length) {
            setIsChecked(true);
        } else {
            setIsChecked(false);
        }
    }, [selectedRows, credentialList?.length]);
                                    ...
                                    <div className="col-lg-6 d-flex align-items-center justify-content-end">
                                        ...
                                        <div className='ms-3 me-3'>
                                            <div className="pseudo-search">
                                                ...
                                                <input
                                                    type="text"
                                                    placeholder="Search Credential Configure"
                                                    value={search}
                                        ...
                                        <button className="add-btn boreder-0 " type="button" onClick={() => createCredentialConfigure()}>
                                            Create
...
export default CredentialConfigure;