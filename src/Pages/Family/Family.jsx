...
const Family = () => {
    ...
    const handleLeave = async () => {
        ...
        try {
            const res = await Axios.post(`/family/leave`, { family_id: leaveId }, authorizationHeaders());
            ...
            console.error("Error-Leave-Family++", err);
    ...
    const handleFamily = (i) => {
        if (i?.org_id != null && i?.org_id != "") { // value 
            navigate("/home2");

            localStorage.setItem("family_name", i?.family_name);
            // localStorage.setItem("openCloudOption", false);
            // localStorage.setItem("openUser", true);

            // localStorage.setItem("user_org_id", i?.org_id);
    ...
    return (
        <>
                                                ...
                                                {familyData?.map((i, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            ...
                                                            {
                                                                (!('is_owner' in i) && i?.org_id === null) && (
                                                                    <button
                                                                        type="button"
                                                                        className="leave boreder-0"
                                                                        onClick={() => {
                                                                            setShow({ ...show, leaveModal: true });
                                                                            setLeaveId(i?.family_id);
                                                                        }}
                                                                    >
                                                                        Leave
                                                                    </button>
                                                            ...
                                                            <img
                                                                src={editbtn}
                                                                alt="Edit"
                                                                className='ms-4 me-4 cursor-pointer'
                                                                onClick={() => {
            ...
            <DeleteVpc show={show.deleteModal} handleClose={handleClose} handleDelete={handleDelete} isDeleteLoading={isDeleteLoading} />
            <Leave show={show.leaveModal} handleClose={handleClose} handleLeave={handleLeave} isLeaveLoading={isLeaveLoading} role={"family"} />
